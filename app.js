...
async function liveBalance(){try{const r=await window.controllerAPI.kalshiBalance();const cents=Number(r.balance);$('liveStatus').textContent=Number.isFinite(cents)?`LIVE BALANCE: ${(cents/100).toFixed(2)}`:'BALANCE RESPONSE RECEIVED';return cents}catch(e){$('liveStatus').textContent='BALANCE ERROR: '+e.message;return null}}
