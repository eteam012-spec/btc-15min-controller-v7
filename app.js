...
let autoLive=false,noTradeWindow=false,autoBusy=false,autoEntryDoneTicker=null,autoPosition=null,lastAutoActionAt=0,dailyLossCents=0,autoDay=day(),autoRealizedBaselineCents=null;
...
function autoResetDay(){if(autoDay!==day()){autoDay=day();dailyLossCents=0;autoRealizedBaselineCents=null;}}
...
async function autoReconcile(){if(!autoLive||!liveTicker)return;try{const p=await window.controllerAPI.kalshiPositions(undefined, Number.isInteger(liveMarket?.exchangeIndex)?liveMarket.exchangeIndex:undefined);const positions=Array.isArray(p?.market_positions)?p.market_positions:[];const realizedCents=positions.reduce((sum,v)=>sum+(Number(v.realized_pnl_dollars)||0)*100,0);if(autoRealizedBaselineCents===null)autoRealizedBaselineCents=realizedCents;dailyLossCents=Math.max(0,Math.round(autoRealizedBaselineCents-realizedCents));autoPosition=autoPositionFromResponse(p);$('autoStatus').textContent=`AUTO LIVE: ${autoLive?'ON':'OFF'} • NO-TRADE: ${noTradeWindow?'ON':'OFF'} • POSITION: ${autoPosition?(autoPosition.yesPosition>0?'YES/UP':'NO/DOWN'):'FLAT'}`;}catch(e){$('autoStatus').textContent='AUTO LIVE: '+e.message}}
...
async function armAuto(){try{const p=await window.controllerAPI.kalshiPositions(undefined, Number.isInteger(liveMarket?.exchangeIndex)?liveMarket.exchangeIndex:undefined);const positions=Array.isArray(p?.market_positions)?p.market_positions:[];autoRealizedBaselineCents=positions.reduce((sum,v)=>sum+(Number(v.realized_pnl_dollars)||0)*100,0);dailyLossCents=0;await window.controllerAPI.kalshiAutoArm();autoLive=true;noTradeWindow=false;$('autoStatus').textContent='AUTO LIVE: ON • watching for eligible entries';}catch(e){$('autoStatus').textContent='AUTO ARM ERROR: '+e.message}}
