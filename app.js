...
let minute=1,windowId='',windowStart='',rows=[],allRecords=[];
let liveTicker=null, liveMarket=null, lastLiveAt=0, polling=null, switching=false;
...
function startWindow(market){minute=1;windowId='W-'+Date.now();windowStart=new Date().toISOString();rows=[];paperPosition=null;liveMarket=market||liveMarket;$('btc').value=DEFAULTS.btc;$('target').value=extractTarget(liveMarket) ?? DEFAULTS.target;render();calc()}
...
function calc(){
 ...
 const dataFresh=lastLiveAt>0&&Date.now()-lastLiveAt<10000;let pick=score>=0?'UP':'DOWN',confidence=clamp(50+Math.abs(score)*1.25,0,99),risk='MODERATE',action='WATCH';
...
async function record(){const r=calc();const row={id:`${windowId}:${Date.now()}`,type:'OBSERVATION',date:day(),timestamp:new Date().toISOString(),windowId,windowStart,minute,...r,result:'PENDING'};rows.push(row);render();await persist()}
...
function summarizeWindow(rs,settlement){const obs=rs.filter(r=>r.type==='OBSERVATION'&&r.components);if(!obs.length)return null;const last=obs[obs.length-1],avg=k=>obs.reduce((s,r)=>s+(Number(r[k])||0),0)/obs.length;const comps={};for(const k of Object.keys(BASE_WEIGHTS))comps[k]=avgComp(obs,k);const pick=last.pick;const correct=pick===settlement;return {windowId:last.windowId,windowStart:last.windowStart,ticker:last.marketTicker,settlement,pick,correct,score:last.score,confidence:last.confidence,components:comps,observations:obs.length,completedAt:new Date().toISOString()}}
...
function isOpen(m){return m&&String(m.status||'').toLowerCase()==='open'}
