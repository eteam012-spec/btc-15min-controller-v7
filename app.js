...
function autoPositionFromResponse(p){const list=p?.market_positions||[];const a=Array.isArray(list)?list:[p];const x=a.find(v=>String(v.ticker||v.market_ticker||'')===liveTicker)||a[0];if(!x)return null;const pos=Number(x.position_fp??x.position??0);return Number.isFinite(pos)&&pos!==0?{ticker:liveTicker,yesPosition:pos}:null}
